...
diabetic_data=pd.read_csv('diabetic.csv')
bodyfat_data=pd.read_csv('bodyfatbmi.csv')
healthcare_data = pd.read_csv('healthcaredataset.csv')
diet_dataset=pd.read_csv('diet_dataset.csv')
...
healthcare_data_cleaned.to_csv('healthcaredataset_cleaned.csv', index=False)
diet_dataset.to_csv('diet_dataset_cleaned.csv', index=False)

def show_entry_fields(data):
    return "Age: %s  Veg-NonVeg: %s Weight: %s kg Hight: %s cm" % (data['age'], data['Veg-NonVeg'],data['weight'], data['height'])
...
############### MODEL PREDICTION FOR BODYFAT
scaler2 = StandardScaler()
# Fit the scaler with only BMI data
x1 = bodyfat_data[['BMI','Weight_kg','Height_cm','Age']]
scaler2.fit(x1)

scaler2 = StandardScaler()
features_scaled = scaler2.fit_transform(bodyfat_data[['BMI']])
# X_normalized = scaler2.fit_transform(bodyfat_data[['BMI', 'Height_m', 'Weight_kg', 'Age']])
y1 = bodyfat_data['BodyFat']

X_train1, X_test1, y_train1, y_test1 = train_test_split(features_scaled, y1, test_size=0.2, random_state=42)
...
model_f.compile(optimizer='adam', loss='mean_squared_error', metrics=['accuracy'])
model_f.fit(X_train1, y_train1, epochs=10, batch_size=32)
loss, accuracy = model_f.evaluate(X_test1, y_test1)
print(f"Model Bodyfat Accuracy: {accuracy}")
...
#####################################################################""
def predict_bodyfat_percentage(bmii):
    # Assuming user_data is a dictionary with keys like 'BMI', 'Height', 'Weight', 'Age'
    # Extract and order the values as expected by the model
    scaled_bmi = scaler2.transform([[bmii]])

    # Predicting the body fat
    predicted_body_fat = model_f.predict(scaled_bmi)

    return predicted_body_fat[0]

def Healthy(user_data):

    # Assuming bmiCalculation function returns a tuple, and the first element is the BMI value
    _,_,_,health_status,bmiValue = bmi.bmiCalculation(user_data)

    # Filter based on Veg-NonVeg preference
    veg_nonveg_filter = user_data['Veg-NonVeg']
    ...
    bodyfact_probability_percentage=float(bodyfact_probability)
